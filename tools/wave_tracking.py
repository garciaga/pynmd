...
def wave_tracks(local_extrema,ot_lag,twind,wh=None):
    ...
    for aa in range(np.min(num_waves)):
        ...
        for bb in range(1,ot_lag.shape[1]):
            ...
            # If the time corresponding to the minimum is within the window of 
            # interest then allocate the value. Otherwise discard it and exit
            # the inner loop. Returning NANs for the rest of the values as we
            # have lost the ability to tack that particular wave. 
            if np.min(tmp_dt) > twind:
                break
            else:
